...
class ControllerExtensionPaymentNOCHEX extends Controller {
	...
	public function index() {
		...
		if ($this->request->post['payment_nochex_api'] == 1) { 
			if (isset($this->request->post['payment_nochex_api_key']) == ""){
			$this->response->redirect($this->url->link('extension/payment/nochex', 'user_token=' . $this->session->data['user_token'] . '&type=payment', true));
			}else{			
		...
		$data['entry_emailusd'] = $this->language->get('entry_emailusd');
		$data['entry_emaileur'] = $this->language->get('entry_emaileur');
		...
		if (isset($this->error['api'])) {
			$data['error_apikey'] = $this->error['api'];
		} else {
			$data['error_apikey'] = '';
		}
		...
		if (isset($this->request->post['payment_nochex_merchantusd'])) {
			$data['payment_nochex_merchantusd'] = $this->request->post['payment_nochex_merchantusd'];
		} else {
			$data['payment_nochex_merchantusd'] = $this->config->get('payment_nochex_merchantusd');
		}
		if (isset($this->request->post['payment_nochex_merchanteur'])) {
			$data['payment_nochex_merchanteur'] = $this->request->post['payment_nochex_merchanteur'];
		} else {
			$data['payment_nochex_merchanteur'] = $this->config->get('payment_nochex_merchanteur');
		}
		...
		if (isset($this->request->post['payment_nochex_api'])) { 
			$data['payment_nochex_api'] = $this->request->post['payment_nochex_api'];
		} else { 
			$data['payment_nochex_api'] = $this->config->get('payment_nochex_api');
		}		
		
		if (isset($this->request->post['payment_nochex_api_key'])) { 
			$data['payment_nochex_api_key'] = $this->request->post['payment_nochex_api_key'];
		} else { 
			$data['payment_nochex_api_key'] = $this->config->get('payment_nochex_api_key');
		}

		if (isset($this->request->post['payment_nochex_apiRedirect'])) { 
			$data['payment_nochex_apiRedirect'] = $this->request->post['payment_nochex_apiRedirect'];
		} else { 
			$data['payment_nochex_apiRedirect'] = $this->config->get('payment_nochex_apiRedirect');
		}
