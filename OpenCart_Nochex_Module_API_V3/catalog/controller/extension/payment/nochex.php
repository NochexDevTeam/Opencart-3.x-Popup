...
class ControllerExtensionPaymentNochex extends Controller {
	public function index() {
		...
		foreach ($products as $product) {
			$description .= " Product : ".preg_replace("/[^A-Za-z0-9 ]/", "", $product['name']).", ".$product['quantity']." x " . $product['price'] . "   ";
		}
		...
		if ($order_info['currency_code'] == "EUR"){
			...
			$data['merchant_id'] = $this->config->get('payment_nochex_merchanteur');	
	
		} else if ($order_info['currency_code'] == "USD"){
		
			$data['merchant_id'] = $this->config->get('payment_nochex_merchantusd');
		...
		$data['billing_postcode'] = preg_replace("/[^A-Za-z0-9 ]/", "", $order_info['payment_postcode']);
		...
		if ($this->cart->hasShipping()) {
			...
			$data['delivery_postcode'] = preg_replace("/[^A-Za-z0-9 ]/", "", $order_info['shipping_postcode']);
		} else {
			$data['delivery_fullname'] = preg_replace("/[^A-Za-z ]/", "", $order_info['payment_firstname']) . ' ' . preg_replace("/[^A-Za-z ]/", "", $order_info['payment_lastname']);
			...
			$data['delivery_postcode'] = preg_replace("/[^A-Za-z0-9 ]/", "", $order_info['payment_postcode']);
		}
		...
		if($this->config->get('payment_nochex_api')==1 AND $this->config->get('payment_nochex_api_key') <> "" AND $order_info['currency_code'] == "GBP"){
		$data['apiShow'] = $this->config->get('payment_nochex_api');
		$data['api_key'] = $this->config->get('payment_nochex_api_key');
		$data['apiRedirect'] = $this->config->get('payment_nochex_apiRedirect');
		}else{		
		$data['apiShow'] = "0";
		$data['api_key'] = "";
		$data['apiRedirect'] = "";
	...
	public function callback() {
		...
		$Msg = "APC was " . $output. ", and this was a " . $_POST['status'] . " transaction. Transaction ID - " . $_POST["transaction_id"];
			$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_nochex_order_status_id'), $Msg, false);
